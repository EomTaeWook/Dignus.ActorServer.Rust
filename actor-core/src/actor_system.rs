...
impl ActorSystem {
    ...
    pub fn spawn_with_capacity<TActor>(self: &Arc<Self>, mailbox_capacity: usize) -> Arc<dyn ActorRefTrait>
    where
        TActor: ActorBase + Default + 'static,
    {
    ...
    pub fn spawn_with_options<TActor>(self: &Arc<Self>, alias: Option<String>, mailbox_capacity: usize) -> Arc<dyn ActorRefTrait>
    where
        TActor: ActorBase + Default + 'static,
    {
    ...
    pub fn spawn_with_factory<TActor, TFactory>(self: &Arc<Self>, factory: TFactory) -> Arc<dyn ActorRefTrait>
    where
    ...
    pub fn spawn_with_factory_and_alias<TActor, TFactory>(self: &Arc<Self>, factory: TFactory, alias: String) -> Arc<dyn ActorRefTrait>
    where
        TActor: ActorBase + 'static,
        TFactory: FnOnce() -> TActor,
    {
    ...
    pub fn spawn_with_factory_and_capacity<TActor, TFactory>(self: &Arc<Self>, factory: TFactory, mailbox_capacity: usize) -> Arc<dyn ActorRefTrait>
    where
        TActor: ActorBase + 'static,
        TFactory: FnOnce() -> TActor,
    {
    ...
    pub fn spawn_with_factory_options<TActor, TFactory>(self: &Arc<Self>, factory: TFactory, alias: Option<String>, mailbox_capacity: usize) -> Arc<dyn ActorRefTrait>
    where
        TActor: ActorBase + 'static,
        TFactory: FnOnce() -> TActor,
    {
    ...
    pub fn spawn_on_dispatcher<TActor>(self: &Arc<Self>, dispatcher_index: usize) -> Arc<dyn ActorRefTrait>
    where
        TActor: ActorBase + Default + 'static,
    {
        self.spawn_on_dispatcher_with_options::<TActor>(dispatcher_index, None, DEFAULT_MAILBOX_CAPACITY)
    }

    pub fn spawn_on_dispatcher_with_alias<TActor>(self: &Arc<Self>, dispatcher_index: usize, alias: String) -> Arc<dyn ActorRefTrait>
    where
        TActor: ActorBase + Default + 'static,
    {
        self.spawn_on_dispatcher_with_options::<TActor>(dispatcher_index, Some(alias), DEFAULT_MAILBOX_CAPACITY)
    }

    pub fn spawn_on_dispatcher_with_capacity<TActor>(self: &Arc<Self>, dispatcher_index: usize, mailbox_capacity: usize) -> Arc<dyn ActorRefTrait>
    where
        TActor: ActorBase + Default + 'static,
    {
    ...
    pub fn spawn_on_dispatcher_with_options<TActor>(self: &Arc<Self>, dispatcher_index: usize, alias: Option<String>, mailbox_capacity: usize) -> Arc<dyn ActorRefTrait>
    where
        TActor: ActorBase + Default + 'static,
    {
    ...
    pub fn spawn_on_dispatcher_with_factory<TActor, TFactory>(self: &Arc<Self>, factory: TFactory, dispatcher_index: usize) -> Arc<dyn ActorRefTrait>
    where
        TActor: ActorBase + 'static,
        TFactory: FnOnce() -> TActor,
    {
        self.spawn_on_dispatcher_with_factory_options(factory, dispatcher_index, None, DEFAULT_MAILBOX_CAPACITY)
    }

    pub fn spawn_on_dispatcher_with_factory_and_alias<TActor, TFactory>(self: &Arc<Self>, factory: TFactory, dispatcher_index: usize, alias: String) -> Arc<dyn ActorRefTrait>
    where
        TActor: ActorBase + 'static,
        TFactory: FnOnce() -> TActor,
    {
        self.spawn_on_dispatcher_with_factory_options(factory, dispatcher_index, Some(alias), DEFAULT_MAILBOX_CAPACITY)
    }

    pub fn spawn_on_dispatcher_with_factory_and_capacity<TActor, TFactory>(self: &Arc<Self>, factory: TFactory, dispatcher_index: usize, mailbox_capacity: usize) -> Arc<dyn ActorRefTrait>
    where
        TActor: ActorBase + 'static,
        TFactory: FnOnce() -> TActor,
    {
        self.spawn_on_dispatcher_with_factory_options(factory, dispatcher_index, None, mailbox_capacity)
    }

    pub fn spawn_on_dispatcher_with_factory_options<TActor, TFactory>(self: &Arc<Self>, factory: TFactory, dispatcher_index: usize, alias: Option<String>, mailbox_capacity: usize) -> Arc<dyn ActorRefTrait>
    where
        TActor: ActorBase + 'static,
        TFactory: FnOnce() -> TActor,
    {
    ...
    pub fn try_get_actor_ref_by_alias(self: &Arc<Self>, alias: &str) -> Option<Arc<dyn ActorRefTrait>> {
        let handle = {
        ...
        let actor_ref = ActorRef::new(index, generation, Some(alias.to_string()), Arc::clone(&self.registry));
        Some(Arc::new(actor_ref))
    }

    pub(crate) fn spawn_with_dispatcher<TActor>(self: &Arc<Self>, actor: TActor, dispatcher_index: usize, alias: Option<String>, mailbox_capacity: usize) -> Arc<dyn ActorRefTrait>
    where
        TActor: ActorBase + 'static,
    {
        ...
        if dispatcher_index >= self.dispatchers.len() {
            panic!("dispatcher_index is out of range. dispatcher_index:{}", dispatcher_index);
        }

        self.register_actor(actor, Some(dispatcher_index), alias, mailbox_capacity)
    }

    pub(crate) fn spawn_with_auto_dispatcher<TActor>(self: &Arc<Self>, actor: TActor, alias: Option<String>, mailbox_capacity: usize) -> Arc<dyn ActorRefTrait>
    where
        TActor: ActorBase + 'static,
    {
        self.throw_if_disposed();
    ...
    fn register_actor<TActor>(self: &Arc<Self>, actor: TActor, dispatcher_index: Option<usize>, alias: Option<String>, mailbox_capacity: usize) -> Arc<dyn ActorRefTrait>
    where
        TActor: ActorBase + 'static,
    {
